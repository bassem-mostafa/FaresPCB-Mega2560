...
void setup()
{
    ...
    LCD_Setup(LCD_Custom_Character_1, LCD_Icon_LockOn);
    LCD_Setup(LCD_Custom_Character_2, LCD_Icon_LockOff);
    LCD_LightOn();
}

void loop()
{
    static LCD_Screen_t LCD_Screen_Lock =
    {
            "  Lock Screen   ",
            {
                    // "??? ??? ?? ??:??:?? ????"
    ...
    switch (LCD_Screen_Lock[1][0])
    {
        case LCD_Custom_Character_1:
            LCD_Screen_Lock[1][0] = LCD_Custom_Character_2;
            memcpy(&LCD_Screen_Lock[1][1], "    Unlocked    ", strlen("    Unlocked    "));
            break;
        case LCD_Custom_Character_2:
            LCD_Screen_Lock[1][0] = LCD_Custom_Character_1;
            memcpy(&LCD_Screen_Lock[1][1], "     Locked     ", strlen("     Locked     "));
            break;
        default:
            memcpy(&LCD_Screen_Lock[1][0], "     Error      ", strlen("     Error      "));
            break;
    }
    LCD_Write(LCD_Screen_Lock);
    _delay_ms(2000);
}
